...
const registerUser = async () => {
  ...
  //添加用户
  await axios
    .post("/api/addUser", register)
    .then((res) => {
      console.log(res);
      user_id.value = res.data.insertId;
  ...
  await axios
    .post(`/api/addHead?user_id=${user_id.value}`)
    .then((res) => {
      console.log(res);
    })
    .catch((err) => {
...
const loginUser = async () => {
  ...
  await axios
    ...
    .then((res) => {
      ...
      if (res.data == "没有找到对应的用户") alert("用户名或密码错误");
    })
    .catch((err) => {
      console.log(err);
    });
